#Mateusz Waga algorytm łaczy dwa rodzaje sortowań(insert i merge sort) dla k<=10 robi insert(O(n)=n*k) a dla k>10 merge(O(n)=n*logk)
...
def SortM(l,k): #merge sort dla k
    ...
    while l is not None:
        ...
        if ile==k:
            ...
            p = l.next
            l.next = None
            head=scal(head,sort_m(podzial(g)))
            for i in range(k):
                head=head.next
            l=p
            g=p
